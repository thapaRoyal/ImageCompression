...
export function compressImage(fileOrBlob, options) {
    return __awaiter(this, void 0, void 0, function* () {
        const { maxSizeMB = 0.1, quality = 0.9, maxWidth = 800, maxHeight = null, downscaleDivisor = 5, } = options;
        ...
        const imageBitmap = yield createImageBitmap(fileOrBlob);
        let origWidth = imageBitmap.width;
        let origHeight = imageBitmap.height;
        if (origWidth > maxWidth * downscaleDivisor ||
            ...
            console.log(`Quick downscale by divisor ${downscaleDivisor} to ${origWidth}x${origHeight}`);
        }
        ...
        const encodeWithBestQuality = (canvas) => __awaiter(this, void 0, void 0, function* () {
            let low = minQuality;
            let high = quality;
            let bestBlob = null;
            while (low <= high) {
                const mid = (low + high) / 2;
                const blob = yield new Promise((resolve) => canvas.toBlob(resolve, format, mid));
                if (!blob)
                    throw new Error(`Compression failed for ${format}`);
        ...
        const drawToCanvas = (width, height) => {
            const canvas = document.createElement('canvas');
            canvas.width = width;
        ...
        while (!bestBlob) {
            ...
            bestBlob = yield encodeWithBestQuality(canvas);
            if (!bestBlob || bestBlob.size > maxSizeBytes) {
                console.warn(`Still too big (${bestBlob ? (bestBlob.size / 1024).toFixed(1) : '??'}KB), downscaling further`);
        ...
        console.log(`✅ Final size: ${(bestBlob.size / 1024).toFixed(1)}KB, Dimensions: ${newWidth}x${newHeight}`);
        return file;
    });
}
